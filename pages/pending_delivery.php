...
$staff_options = '';
$result_staff = $conn->query("SELECT staff_id, staff_name FROM staff ORDER BY staff_name");
if ($result_staff) {
...
$vehicle_options = '';
$result_vehicles = $conn->query("SELECT vehicle_id, CONCAT(vehicle_name, ' (', vehicle_plate, ')') AS vehicle_info FROM vehicles ORDER BY vehicle_name");
if ($result_vehicles) {
    while ($row = $result_vehicles->fetch_assoc()) {
        $vehicle_options .= "<option value='" . htmlspecialchars($row['vehicle_id']) . "'>" . htmlspecialchars($row['vehicle_info']) . "</option>";
    }
}
...
<body>
    ...
    <script>
        ...
        function renderTable(orders) {
            ...
            if (!orders.length) {
                tbody.html('<tr><td colspan="11" class="text-center py-4">ไม่พบข้อมูล</td></tr>');
                return;
            }
                ...
                tbody.append(`
                    <tr id="order-row-${row.order_id}" class="priority-${row.priority || ''}">
                        ...
                        <td class="action-buttons">
                            <button class="btn btn-secondary btn-sm view-details-btn" data-orderid="${row.order_id}">
                                <i class="fas fa-eye"></i> ดูรายละเอียด
                            </button>
                            <button class="btn btn-warning btn-sm confirm-delivery-btn" data-orderid="${row.order_id}" data-docno="${escapeAttr(row.cssale_docno || '')}">
                                <i class="fas fa-truck-loading"></i> ยืนยันการส่ง
                            </button>
                            <button class="btn btn-info btn-sm edit-driver-btn"
                                ...
                                data-current-staff="${escapeAttr(row.assigned_staff_name || '')}"
                                data-current-vehicle="${escapeAttr(row.assigned_vehicle_info || '')}"
                                data-current-staff-id="${row.assigned_staff_id || ''}"
                            ...
                            <div class="dropdown d-inline-block">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <?php if (has_role([3, 4])): ?>
                                    <button class="dropdown-item rollback-status-btn" type="button" data-orderid="${row.order_id}" data-docno="${escapeAttr(row.cssale_docno || '')}" data-target-status="รับเรื่อง">
        ...
        $(document).ready(function() {
            ...
            $('#orders-table-body').on('click', '.confirm-delivery-btn', function() {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                        url: '<?php echo BASE_URL; ?>php/confirm_delivery.php',
                        type: 'POST',
                        dataType: 'json',
            ...
            $('#orders-table-body').on('click', '.edit-driver-btn', function() {
                $('#editOrderId').val($(this).data('orderid'));
                $('#editOrderInfo').val(`${$(this).data('docno')} - ${$(this).data('custname')}`);
                $('#editCurrentStaff').val($(this).data('current-staff') || '-');
                $('#editCurrentVehicle').val($(this).data('current-vehicle') || '-');
                $('#editCurrentStaffId').val($(this).data('current-staff-id') || '');
                $('#editCurrentVehicleId').val($(this).data('current-vehicle-id') || '');
                $('#newStaff').val($(this).data('current-staff-id') || '');
                $('#newVehicle').val($(this).data('current-vehicle-id') || '');
            ...
            $('#saveDriverBtn').on('click', function() {
                ...
                if (!newStaffId || !newVehicleId) {
                    Swal.fire({ icon: 'warning', title: 'กรุณาเลือกข้อมูลให้ครบ', text: 'โปรดเลือกทั้งคนขับและรถ' });
                    return;
                }

                if (newStaffId === currentStaffId && newVehicleId === currentVehicleId) {
                    Swal.fire({ icon: 'info', title: 'ไม่มีการเปลี่ยนแปลง', text: 'ข้อมูลคนขับและรถยังเหมือนเดิม' });
                    return;
                }
                ...
                Swal.fire({
                    ...
                    html: `คุณต้องการเปลี่ยนคนขับรถสำหรับออเดอร์: <strong>${$('#editOrderInfo').val()}</strong><br><br>
                           ...
                           ไปเป็น: <strong>${$('#newStaff option:selected').text()} / ${$('#newVehicle option:selected').text()}</strong><br><br>
                           ใช่หรือไม่?`,
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            Swal.close();
                            if (response.status === 'success') {
                                Swal.fire({ icon: 'success', title: 'เปลี่ยนคนขับสำเร็จ!', text: response.message, timer: 2000, showConfirmButton: false })
                                    .then(() => {
